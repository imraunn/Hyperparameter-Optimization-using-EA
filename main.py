...
class CustomClassifier:
    def __init__(self, classifier, param_choices):
    ...
    def train(self, x_train, y_train, x_test, y_test):
        ...
        self.f1_score = f1_score(y_test, y_pred)  # Update to calculate F1 score
...
class Optimizer:
    ...
    def create_population(self, count, classifier, x_train, y_train, x_test, y_test):
        population = []
        for _ in range(count):
            custom_classifier = CustomClassifier(classifier, self.param_choices)
            custom_classifier.create_random()
            custom_classifier.train(x_train, y_train, x_test, y_test)  # Evaluate each classifier upon creation
    ...
    def breed(self, mother, father, x_train, y_train, x_test, y_test):
        children = []
        child1, child2 = {}, {}
        ...
        # Test the offspring classifiers
        classifier1.train(x_train, y_train, x_test, y_test)
        classifier2.train(x_train, y_train, x_test, y_test)
    ...
    def evolve(self, pop, x_train, y_train, x_test, y_test):
        ...
        while len(children) < desired_length:
            ...
            if male != female:
                ...
                babies = self.breed(male, female, x_train, y_train, x_test, y_test)  # Add X and y for testing
                for baby in babies:
...
class EvolutionaryProcess:
    ...
    def evolve(self, x_train, y_train, x_test, y_test):
        ...
        self.classifiers = optimizer.create_population(self._population, self.classifier, x_train, y_train, x_test, y_test)

        for _ in tqdm(range(self._generations), desc="Evolution Progress"):
            self._train_classifiers(x_train, y_train, x_test, y_test)
            self.classifiers = optimizer.evolve(self.classifiers, x_train, y_train, x_test, y_test)
    ...
    def _train_classifiers(self, x_train, y_train, x_test, y_test):
...
evolution.evolve(x_train, y_train, x_test, y_test)
